...
class pedidos extends CI_model {
    ...
    function cp_condicoes($id = 0) {
        ...
        array_push($cp, array('$A', '', 'Dados da empresa', False, True));
        array_push($cp, array('$Q id_fi:fi_nome_fantasia:select * from _filiais where fi_ativo = 1 order by fi_nome_fantasia', 'pp_filial', 'Empresa', True, True));
        if (perfil('#MUL')) {
            array_push($cp, array('$Q id_us:us_nome:select * from users where us_ativo = 1 order by us_nome', 'pp_vendor', 'Vendedor', True, True));
        }

        /* CONDICOES DE PAGAMENTO */
        array_push($cp, array('$A', '', 'Condições de pagamento', False, True));
    ...
    function le($id) {
        $sql = "select * from " . $this -> table . "
					...
					LEFT JOIN _filiais ON pp_filial = id_fi				
    ...
    function pedido_items($id, $edit = 0) {
        ...
        $sx .= '<tr style="font-size: 75%; border-top: 1px solid #000000;" valign="center">
					...
					<th width="10%" style="border-bottom: 1px solid #808080;" align="center"><b>vlr.unit./diária</b></th>
        ...
        for ($r = 0; $r < count($rlt); $r++) {
            ...
            $sx .= '<td align="center"><span style="font-size: 75%;">' . ($r + 1) . '</span></td>';
    ...
    function contatos_do_pedido_simples($ped, $clie, $editar = 0) {
        $sx = '';
        $ped = round($ped);
